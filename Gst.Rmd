---
title: "Population structure 2: $G_{st}$ in the machine"
...
get_gst <- function(x) Gst_Nei(selPopSize(x, n = 10))

Gst_heatmap <- function(x){
  gst <- pairwise_Gst_Nei(selPopSize(x, n = 10))
  plot <- heat_gst(gst)
  return(plot)
}
```
...
```{r, by_year, cache=TRUE, fig.height = 7, fig.width = 10}
get_gst(ramdat)
Gst_heatmap(ramdat)
```

```{r, by_Zone, cache = TRUE, fig.height = 7, fig.width = 10}
get_gst(setpop(ramdat, ~ZONE2))
Gst_heatmap(setpop(ramdat, ~ZONE2))
get_gst(setpop(ramdat, ~ZONE1))
Gst_heatmap(setpop(ramdat, ~ZONE1))
```

```{r, by_year_no_seb, cache = TRUE, fig.height = 7, fig.width = 10}
noseb <- popsub(setpop(ramdat, ~ZONE2), blacklist = "HunterCr")
get_gst(setpop(noseb, ~Pop))
Gst_heatmap(setpop(noseb, ~Pop))
```

```{r, by_Zone_no_seb, cache = TRUE, fig.height = 7, fig.width = 10}

get_gst(setpop(noseb, ~ZONE2))
Gst_heatmap(setpop(noseb, ~ZONE2))

get_gst(setpop(noseb, ~ZONE1))
Gst_heatmap(setpop(noseb, ~ZONE1))
```
...
```{r, by_Zoneyear, cache = TRUE, fig.height = 7, fig.width = 10}
get_gst(setpop(ramdat, ~ZONE2/Pop))

z2 <- Gst_heatmap(setpop(ramdat, ~ZONE2/Pop))
...
get_gst(setpop(ramdat, ~ZONE1/Pop))
z1 <- Gst_heatmap(setpop(ramdat, ~ZONE1/Pop))
...
```{r, by_Zoneyear_no_seb, cache = TRUE, fig.height = 7, fig.width = 10}

get_gst(setpop(noseb, ~ZONE2/Pop))
z2 <- Gst_heatmap(setpop(noseb, ~ZONE2/Pop))
z2labs <- sub("^.+?_20", "20", unique(z2$data$Var1))
intercepts <- (which(!duplicated(z2labs)) - 0.5)[-1]
...
get_gst(setpop(noseb, ~ZONE1/Pop))
z1 <- Gst_heatmap(setpop(noseb, ~ZONE1/Pop))
z1labs <- sub("^.+?_20", "20", unique(z1$data$Var1))
intercepts <- (which(!duplicated(z1labs)) - 0.5)[-1]
